...
app = Client("Sirebrown", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
print("Bot ready")
...
async def start(client: Client, message: Message):
    ...
    try:
        response = await shadow_ai(prompt)  # awaited here
        if response:  # make sure we have a response
            await message.reply_text(str(response))  # convert to string explicitly
        else:
            await message.reply_text("No response generated")
    except Exception as e:
        print(f"Error in echo handler: {str(e)}")
        await message.reply_text("Désolé, je ne suis pas dispo pour le moment, discutons plus tard.")
